...
## 순간 가벼운 톡이 먼저 오고, 손을 뗄 때 뭉툭한 한 겹이 그것을 닫는다.
##
...
## 손가락이 **닿을 때** 나는 감촉. 눌렀다는 사실만 알리는 가장 가벼운 톡이다.
## `Haptics` 는 오토로드라 상수 초기화 시점에는 없다 — `_ready` 에서 채운다.
var down_kind: int = NONE

## 손가락을 **뗄 때**(= 활성화) 나는 감촉. 뭉툭하게 닫는 한 겹이다.
...
func _ready() -> void:
	down_kind = Haptics.Kind.LIGHT
	up_kind = Haptics.Kind.SOFT
