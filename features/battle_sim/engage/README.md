...
> **약자 멸시(암살 R)는 점수를 아예 건너뛴다.** 스택이 남아 있는 동안은
> **체력이 가장 적은 적**(`_weakest_enemy`, 비율이 아니라 절대값)만 겨눈다 —
> 거리도 존재감도 보지 않는 것이 이 카드의 값이다. 스택은 **한 차례에 하나**
> 준다(`u.contempt_pick` 이 그 차례의 표적을 실제로 이 규칙이 정했는지를 들고
> 있다) — 전탄 발사로 다섯을 때려도 겨눔은 한 번이고, 다른 규칙이 고른 표적에
> 값을 물리면 스택이 겨눔과 무관하게 마른다.
...
| **약자 멸시**(암살 R) | `_pick_target` | 위 타겟 선정 절 참조 |
...
`on_engage_end`(`_finish_engage`)가 걷는다 — 강타 장전 · 약자 멸시 스택 · 불굴
장부 셋. 반응 장갑은 그 짝에 없다: 남은 겹수가 곧 다음 교전까지 가는 값이라
전장을 떠날 때 걷힌다.
