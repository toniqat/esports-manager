...
|---|---|---|
| `OBJ_HERALD_FIRST_TURN` | 12 | 전령이 처음 열리는 턴 |
| `OBJ_DRAGON_FIRST_TURN` | 15 | 용이 처음 열리는 턴 |
| `OBJ_RESPAWN_TURNS` | 15 | **결판이 난 뒤** 다시 열리기까지 |
| `OBJ_RETRY_TURNS` | 10 | **양 팀 미참여로 무산**됐을 때 재시도까지 |
...
## 중립 칸이 오브젝트 자리가 되면서 바뀐 것
좌우 중립 두 칸(`SimulationCore.NEUTRAL_LEFT` / `NEUTRAL_RIGHT`)은 이제
**상시 중립**이다. `SimulationCore.is_objective_cell()` 이 그 판정이고, 정글
규칙에서 통째로 빠진다:

- 캠프가 서지 않는다(`init_jungle_camps`) — 밟아도 성장치가 안 나온다.
- 정글러가 점령할 수 없다(`process_neutral_zone_captures`).
- 정글러의 순회 목표에서 빠진다. **"아직 안 잡힌 중립부터 간다"는 1순위가
  통째로 삭제됐다**(`_nearest_uncaptured_neutral` 함수째로) — 그 판정은 이제
  영원히 참이라, 두면 정글러가 개시부터 끝까지 오브젝트 칸 위에 서서 점령되지도
  않는 칸을 기다리고 캠프는 한 번도 돌지 않는다.
- T1 파괴 보상의 **측면 중립 탈취 분기도 삭제**됐다(`_on_t1_destroyed`) —
  누구의 소유도 되지 않으므로 뺏을 것이 없는 죽은 코드였다.

캠프가 서는 칸이 14 → 12 로 줄었으므로 `BattleSim.SCORE_JUNGLE_CAMP` 을
0.98 → **1.15** 로 올려(14/12 ≈ 1.14) 정글러의 순회 수입 총량을 그대로 두었다.
전령 / 용은 그 위에 얹히는 **추가** 이득이지, 원래 먹던 수입을 되찾는 수단이
아니다.
