...
#### 지속 효과 썸네일 (인게임 탭)
스탯 칩 **바로 아래**. 68×68 칸에 위는 두 글자 약칭(효과별 색), 아래는 지금 값.
아이콘 에셋이 없으므로 **글자가 곧 아이콘**이고, 온전한 이름은 눌러서 여는 정보
패널의 제목이 들고 있다. 칸이 다섯을 넘으면 **줄이 접힌다**(`_build_effect_row`
가 `STAT_W` 안에 들어가는 개수로 나눈다) — 한 줄뿐이던 시절에는 자리도 다섯이
상한이라 정확히 맞아떨어졌지만, 아래 (2)번이 들어오면서 칸 수에 상한이 없어졌다.
