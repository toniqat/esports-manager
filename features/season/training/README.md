...
않는다. 그래서 이 화면은 `Haptics.play(...)` 를 직접 부르고, **세 박자가
점점 무거워지며 한 동작을 닫는다** — 버튼의 두 박자(누름 `LIGHT` → 뗌 `SOFT`)와
같은 문법이다.

| 사건 | 자리 | 감촉 |
|---|---|---|
| 타일을 집어 든다 | `_begin_drag` | `SELECT` |
| 미리보기가 **놓을 수 있는 자리로 막 들어선다** | `_grid_can_drop` | `SELECT` |
| 타일이 판에 물린다 | `_grid_drop` | `SOFT` |
| 판 위의 타일을 탭해서 걷어 낸다 | `_on_grid_input` | `LIGHT` |

**미리보기는 "들어섬"만 운다.** 칸이 바뀌었을 뿐인데도 울리면 판 위를 훑는
내내 진동이 되고, 벗어나는 쪽은 조용하다 — 놓을 수 있게 됐다는 것이 신호다
(`ok and not _hover_ok` 전이 하나만 본다). **걷어내기가 놓기보다 가벼운 것**은
