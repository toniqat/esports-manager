...
# 자리에서 칠하면 검은 원 하나 · 검은 막대 하나가 된다. 그래서 셋은 **`full`
# 아트에서 머리~어깨를 다시 잘라** 만든다 — full 의 알파가 곧 인물 윤곽이라
# 배경이 투명하게 남아 머리 모양과 어깨선이 실루엣으로 읽힌다. `circle` 만
# 불투명한 원 바탕을 구워 넣는다(전장 마커는 뒤에 흰 원을 깔고 교전 아레나는
# 안 까므로, 투명하면 같은 그림이 두 곳에서 다르게 보인다).
#
